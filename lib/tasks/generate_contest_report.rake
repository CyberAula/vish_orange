  ...
  task :report => :environment do
    ...
    Axlsx::Package.new do |p|
      p.workbook.add_worksheet(:name => "Usuarios apuntados al concurso") do |sheet|
        rows = []
        rows << ["Nombre de usuario","Email","Provincia","Código postal","Nombre de la escuela", "Asignatura","Número de teléfono"]
        c = Contest.last
        Contest.last.enrolled_participants.map do |a|
          j = JSON.parse(a.contest_enrollments.first.settings)
          rows << [a.name, a.email, j["additional_fields"]["province"],j["additional_fields"]["postal_code"], j["additional_fields"]["school_name"], j["additional_fields"]["teaching_subject"], j["additional_fields"]["phone_number"]  ]
        end
